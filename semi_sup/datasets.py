...
class ImageDataset(Dataset):
    ...
    def __init__(self, cfg, roots, mode='train', trainsize=538, scale=(0.99, 1.01)):
        self.cfg = cfg
    ...
    # def get_denormed_images(self, index):
    #     image = cv2.imread(self.images[index])
    #     image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    #     mask_3 = cv2.imread(self.gts[index])
    #     if self.cfg.DATA.SEG_CLASSES == 1:
...
def get_dataset(mode, cfg, trainsize=538, scale=(0.99, 1)):
    # print('===mode===>', mode)
    data_root = []
    if "UCL" in cfg.DATA.NAME:
        data_root.append(os.path.join(cfg.DIRS.DATA, 'UCL'))
    if "Endovis" in cfg.DATA.NAME:
        data_root.append(os.path.join(cfg.DIRS.DATA, 'Endovis'))
    if mode == 'train':
        dts = ImageDataset(cfg=cfg, roots=data_root, mode='train', trainsize=trainsize, scale=scale)
        dts2 = ImageDataset_denormed(cfg=cfg, roots=data_root, mode='train', trainsize=trainsize, scale=scale)
        ...
        dataloader_denormed = DataLoader(dts2, batch_sampler=batch_sampler,
                                num_workers=cfg.SYSTEM.NUM_WORKERS, pin_memory=True, worker_init_fn=worker_init_fn)
        ...
        dataloader_denormed = DataLoader(dts, batch_size=batch_size,
                                shuffle=False, drop_last=False,
                                num_workers=cfg.SYSTEM.NUM_WORKERS)
    elif mode == 'test':
    ...
    return dataloader, dataloader_denormed

class ImageDataset_denormed(Dataset):
    """
    dataloader for polyp segmentation tasks
    refer to https://github.com/PRIS-CV/DCRNet/blob/master/utils/dataloader.py
    """
    ...
    def __getitem__(self, index):
        ...
        data_np = self.transform(image=image, mask=mask)
        aug = ToTensorV2()
        data_t = aug(image = data_np['image'], mask=data_np['mask'])
        data = {'imidx': index, 'path': self.images[index], 'image': data_t['image'], 'label': data_t['mask']}
